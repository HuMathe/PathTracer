...
namespace CGL { namespace SceneObjects {
  ...
  void EnvironmentLight::init() {
    ...
    for (int j = 0; j < h; ++j) {
      ...
      this->marginal_y[j] = sum;
      this->conds_y[j * w] = pdf_envmap[w * j] / (sum - temp);
      for (int i = 1; i < w; ++i) 
        this->conds_y[j*w + i] = this->conds_y[j*w+i-1] + pdf_envmap[w*j+i]/(sum-temp);
    }
    if (true)
  ...
  Vector3D EnvironmentLight::sample_L(const Vector3D p, Vector3D* wi,
    ...
    // *distToLight = INF_D;
    // *pdf = 1.0 / (4.0 * PI);
    // return this->bilerp(this->theta_phi_to_xy(this->dir_to_theta_phi(*wi)));

    Vector2D sxy = sampler_uniform2d.get_sample();
    int j = std::upper_bound(this->marginal_y, this->marginal_y + this->envMap->h, sxy.y) - this->marginal_y;
    int i = std::upper_bound(this->conds_y + j*this->envMap->w, this->conds_y + (j+1) * this->envMap->w, sxy.x) - this->conds_y - j * this->envMap->w;
    Vector2D xy (i, j);
    *wi = theta_phi_to_dir(xy_to_theta_phi(xy));
    *distToLight = INF_D;
    *pdf = pdf_envmap[j*this->envMap->w + i] * this->envMap->h * this->envMap->w / 2.0 / PI / PI / sin_theta(*wi);
    return envMap->data[j*envMap->w + i];
  }
  ...
  Vector3D EnvironmentLight::sample_dir(const Ray& r) const {
    ...
    Vector2D xy = this->theta_phi_to_xy(this->dir_to_theta_phi(r.d));
